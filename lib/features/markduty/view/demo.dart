...
class _MarkdutyPageState extends State<MarkdutyPage> {
  ...
  Color timeDisplayColor = Colors.green;
  ...
  void initState() {
    super.initState();
    _loadPunchState(); // Load saved states when app is launched
              ...
              ElevatedButton(
                onPressed: _hasCheckedIn
                  ...
                  setState(() {
                    ...
                    timeDisplayColor = Colors.redAccent;
                  });
                  _savePunchState(); // Save state when IN button is clicked
                },
                child: const Text('IN'),
              ),
              ElevatedButton(
                onPressed: (!_hasCheckedIn || _hasCheckedOut)
                  ...
                  setState(() {
                    _hasCheckedOut = true;
                    _hasCheckedIn = false;
                    timeDisplayColor = Colors.green;
                  });
                  _savePunchState(); // Save state when OUT button is clicked
                },
  ...
  // Load saved check-in and check-out state
  Future<void> _loadPunchState() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();

    // Update the state based on saved values from SharedPreferences
    setState(() {
      _hasCheckedIn = prefs.getBool('hasCheckedIn') ?? false;
      _hasCheckedOut = prefs.getBool('hasCheckedOut') ?? false;

      // Set the appropriate color based on the saved state
      timeDisplayColor = _hasCheckedIn ? Colors.redAccent : Colors.green;
    });
  }

  // Save check-in and check-out state
  Future<void> _savePunchState() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
